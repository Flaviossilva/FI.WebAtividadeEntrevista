    ...
    public class ClienteController : Controller
    {
        ...
        public JsonResult Incluir(ClienteModel model)
        {
            ...
            else
            {
                ...
                {
                    Response.StatusCode = 400;
                    return Json("CPF Cliente ja cadastrado");
                }
                if (model.Beneficiarios != null && model.Beneficiarios.Count > 0)
                {
                    foreach (var beneficiario in model.Beneficiarios)
                    {
                        if (boBeneficiario.VerificarExistencia(beneficiario))
                        {
                            beneficiario.IdCliente = model.Id;  // Associa o cliente recém-criado
                            boBeneficiario.InserirBeneficiario(beneficiario);
                        }
                        else
                        {
                            Response.StatusCode = 400;
                            return Json("CPF Benefeciario ja cadastrado");
                        }
                    }
                }
        ...
        public ActionResult Alterar(ClienteModel model)
        {
            ...
            else
            {
                boCliente.Alterar(new Cliente()
                {
                    ...
                    Sobrenome = model.Sobrenome,
                    Telefone = model.Telefone,
                    CPF = model.CPF//Apenas para demonstração mas o correto seria não alterar o cpf
                });
        ...
        [HttpGet]
        public JsonResult ListaBeneficiario(long idCliente)
        {
